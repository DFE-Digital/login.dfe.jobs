...
const getWebServiceClient = async (application, correlationId) => {
  ...
  const requireAddressing = application.relyingParty.params
    ...
    : false;
  const provisionOrganisationFormatterType =
    application.relyingParty.params.wsProvisionOrganisationFormatterType;
  ...
  if (!applicationClient) {
    ...
    if (provisionOrganisationFormatterType) {
      secureAccessWebServiceClient.setProvisionOrganisationFormatter(
        provisionOrganisationFormatterType,
      );
    }
...
const sendUpdatedUserToApplication = async (
  ...
  try {
    ...
    let localAuthorityName = organisation.localAuthority
      ? organisation.localAuthority.name
      : organisation.name;
    const typeId = organisation.type ? organisation.type.id : undefined;
    const establishmentDfeNumber = organisation.localAuthority
    ...
    const establishmentNumber = organisation.localAuthority
      ? organisation.establishmentNumber
      : undefined;

    await secureAccessWebServiceClient.provisionOrganisation(
      action,
...
const process = async (config, logger, application, data, jobId) => {
  ...
  try {
    ...
    logger.error(
      `Error sending organisation update for ${organisation.id} (legacyId = ${organisation.legacyId}) to ${application.id} - ${e.message}`,
      {
        correlationId,
        stack: e.stack,
      },
