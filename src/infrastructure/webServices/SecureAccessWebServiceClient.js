  ...
  request(rurl, data, callback, exheaders) {
    ...
    asyncRetry(async () => {
      ...
      if (response.status !== 200) {
        ...
        throw new Error(
          `${rurl} returned status ${response.status} - ${responseBody}`,
        );
      }
      ...
      if (
        ...
        !contentTypeHeader.startsWith("application/soap+xml") &&
        !contentTypeHeader.startsWith("Multipart/Related")
      ) {
        throw new Error(
          `${rurl} returned content type ${contentTypeHeader} not text/xml`,
        );
      }
...
class SecureAccessWebServiceClient {
  constructor(
    ...
    targetNamespace,
    provisionUserAction,
    provisionGroupAction,
    provisionOrganisationAction,
    username,
    password,
    requireAddressing,
    correlationId,
    ...
    this._provisionOrganisationFormatter =
      new ProvisionOrganisationDefaultFormatter();
  ...
  async provisionUser(
    ...
    const message = this._provisionUserFormatter.getProvisionUserSoapMessage(
    ...
    if (this._username) {
      message
        .setUsernamePassword(this._username, this._password)
        .setTimestamp(60);
    }
    ...
    await this._httpClient.makeSoapRequest(
      this._endpoint,
      message,
      this._provisionUserAction,
    );
  }

  async provisionGroup(action, id, code, name, status, parentId, parentCode) {
    const message = this._provisionGroupFormatter.getProvisionGroupSoapMessage(
      this._targetNamespace,
      action,
      id,
      code,
  ...
  async provisionOrganisation(
    ...
    const message =
      this._provisionOrganisationFormatter.getProvisionOrganisationSoapMessage(
        this._targetNamespace,
        action,
        establishmentDfeNumber,
        establishmentUrn,
    ...
    await this._httpClient.makeSoapRequest(
      this._endpoint,
      message,
      this._provisionOrganisationAction,
    );
  }
  ...
  static async create(
    ...
    const getOperationSoapAction = (wsdl, operationName) => {
      const addressingNamespaces = [
        "http://www.w3.org/2006/05/addressing/wsdl",
      ];
      const addressingPrefix = Object.keys(wsdl.definitions.$)
        .find((key) => {
          const value = wsdl.definitions.$[key];
          return (
            key.startsWith("xmlns:") &&
            addressingNamespaces.find((ns) => ns === value)
          );
        })
        .substr(6);
    ...
    const provisionOrganisationAction = getOperationSoapAction(
      wsdl,
      "ProvisionOrganisation",
    );
    const endpoint = wsdl.definitions.service[0].port[0].address[0].$.location;

    return new SecureAccessWebServiceClient(
      httpClient,
      endpoint,
      tns,
      provisionUserAction,
