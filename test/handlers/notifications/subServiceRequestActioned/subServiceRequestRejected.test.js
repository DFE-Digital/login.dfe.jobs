...
const {
  getHandler,
} = require("../../../../src/handlers/notifications/subServiceRequestActioned/subServiceRequestRejected");
...
describe("When processing a sub_service_request_rejected job", () => {
  ...
  it.each([
    ...
    ["", "", false],
    [" ", "", false],
    [data.reason, data.reason, true],
  ])(
    'should send an email with expected personalisation data, when the reason is "%s" it should be "%s" and the header should be visible %s',
    async (reasonText, expectedReasonText, expectedShowReasonHeader) => {
      const handler = getHandler(config);
