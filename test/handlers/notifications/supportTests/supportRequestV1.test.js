...
describe("When handling supportrequest_v1 job", () => {
  ...
  it.each([
    ...
    ["   ", "", false],
    [undefined, "", false],
    ["mock-some-additional-info", "mock-some-additional-info", true],
  ])(
    "should send an email with expected personalisation data, with the typeAdditionalInfo set as %s, the expected typeAdditionalInfo should be %s and the header should be visible %s",
    async (
      typeAdditionalInfo,
      expectedTypeAdditionalInfo,
      expectedShowAdditionalInfoHeader,
    ) => {
      const handler = getHandler(config);
  ...
  it.each([
    ...
    [null, ""],
    [undefined, ""],
    ["mock-some-urn", "mock-some-urn"],
  ])(
    "should send an email with expected personalisation data, with the urn set as %s, the expected urn should be %s",
    async (urn, expectedUrn) => {
      const handler = getHandler(config);
  ...
  it.each([
    ...
    "should send an email with expected personalisation data, with the orgName set as %s, the expected orgName should be %s",
    async (orgName, expectedOrgName) => {
      const handler = getHandler(config);
