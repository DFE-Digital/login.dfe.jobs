...
describe("When sending email using DiskAdapter", () => {
  ...
  test("then it should write param to disk", async () => {
    ...
    expect(fsWriteFile.mock.calls[0][0]).toMatch(
      /\/temp\/app_data\/email\/unit-tests\/[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}-[0-9]{2}-[0-9]{2}\.json/,
    );
  ...
  test("then it should suppress errors for folder already existing", async () => {
    ...
    await expect(
      adapter.send(recipient, template, data),
