...
describe('MonitorBull', () => {
  ...
  it('should start monitoring the mappings when started', async () => {
    ...
    expect(Worker).toHaveBeenCalledWith('test', expect.any(Function), { connection: { url: 'redis://127.0.0.1:6379' } });
    expect(logger.debug).toHaveBeenCalledWith('MonitorBull: start monitoring test');
  });
