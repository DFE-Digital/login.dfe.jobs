...
describe("when getting template formats", () => {
  ...
  beforeEach(() => {
    ...
    getTemplateFormats =
      require("../../../src/infrastructure/templates").getTemplateFormats;
  });
  ...
  it("then it should return html content type", async () => {
    ...
    expect(actual[0].contentTypes[0].contents).toBe(
      "This is a <b>HTML</b> email for <%=token%>",
    );
  });
  ...
  it("then it should return text content type", async () => {
    ...
    expect(actual[0].contentTypes[1].contents).toBe(
      "This is a text email for <%=token%>",
    );
  });
});
